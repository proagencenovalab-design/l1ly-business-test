...
def is_wellbeing_question(message):
    """Detect direct wellbeing questions and common chat variants."""
    msg = (message or "").lower().strip().replace("’", "'")
    ...
    if any(re.search(pattern, msg) for pattern in direct_patterns):
        return True
...
def wellbeing_reply(message):
    msg = (message or "").lower().strip().replace("’", "'")

    if re.search(r"\b(fine|good|okay|alright|not bad)\b", msg):
        return random.choice([
            "i’m good too, just taking it easy",
            "i’m good too, a little tired though",
            "pretty good too, just relaxing"
        ])
...
def call_openai(prompt):
    ...
    if not reply:
        ...
        raise RuntimeError(
            f"OpenAI a renvoyé une réponse vide "
            f"(status={status}, response_id={response_id})."
        )
