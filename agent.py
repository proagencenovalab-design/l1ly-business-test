...
BOT_VERSION = "v1.0-stable-intelligent-sales"
...
def choose_stage(user: Dict[str, Any], message: str, score: int) -> str:
    ...
    if intent == "blocked":
        return "blocked"
    if int(user.get("age_confirmed", 0)) == 0 and (not non_adult_operational):
        return "age_gate"
...
def is_timewaster_behavior(user: Dict[str, Any], message: str, history: List[Dict[str, str]]) -> bool:
    ...
    free_demands = sum(1 for m in recent_user if any(x in m for x in ["free", "gratuit", "preview", "apercu", "aperçu", "send me", "montre moi"]))
    insults = sum(1 for m in recent_user if any(x in m for x in ["bot", "fucking", "fuck you", "scam", "arnaque"]))
    link_refusals = sum(1 for m in recent_user if any(x in m for x in ["no link", "pas de lien", "ici c'est bien", "stay here"]))

    if "wants_free" in objections and free_demands >= 2:
        return True
    if buyer_profile == "free_seeker" and free_demands >= 2:
        return True
    if insults >= 2:
        return True
...
def should_skip_timewaster_reply(user: Dict[str, Any], message: str, history: List[Dict[str, str]]) -> bool:
    if not is_timewaster_behavior(user, message, history):
        return False
    message_count = int(user.get("message_count") or 0)
    return message_count >= 6 and message_count % 3 == 0
