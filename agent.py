...
# Canonical product URLs are used as safe fallback because old Whop checkout/plan URLs can expire.
CANONICAL_STARTER_LINK = "https://whop.com/lily-novalab/lily-rose-photo-pack-s"
CANONICAL_PREMIUM_LINK = "https://whop.com/lily-novalab/premium-pack-xl-bc/"

def normalize_offer_link(raw_link: str, fallback_link: str) -> str:
    raw_link = (raw_link or "").strip()

    if not raw_link:
        return fallback_link

    # Old checkout/plan links often break with "Nothing to see here yet".
    broken_patterns = [
        "/checkout/plan_",
        "checkout/plan_",
        "plan_",
    ...
    if any(pattern in raw_link for pattern in broken_patterns):
        return fallback_link
...
STARTER_LINK = normalize_offer_link(
    WHOP_STARTER_LINK or DIRECT_PRODUCT_LINK,
    CANONICAL_STARTER_LINK,
)

PREMIUM_PRICE = "75€"
PREMIUM_CONTENT = "3 vidéos + 5 images"
PREMIUM_LINK = normalize_offer_link(
    WHOP_PREMIUM_LINK,
    CANONICAL_PREMIUM_LINK,
)
...
def short_offer_reply(message: str, language: str, user: Dict[str, Any]) -> str:
    ...
    if offer["name"].lower() == "premium":
        return f"if you want the full one, Premium is {offer['price']}: 3 videos + 5 images. {link}"
    return f"start with Starter: {offer['price']}, 4 NSFW images + 1 solo video. {link}"
