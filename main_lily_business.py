...
def _process_message_inner(message, is_business=False):
    ...
    if result.get("skip_send"):
        print("Réponse ignorée volontairement : skip stratégique / age gate / timewaster.", flush=True)
        with get_chat_lock(chat_id):
            ...
            update_user(
                ...
                sexual_profile=result.get("sexual_profile", latest_skip.get("sexual_profile", "")),
                buyer_profile=result.get("buyer_profile", latest_skip.get("buyer_profile", "")),
                fantasies_detected=result.get("fantasies_detected", latest_skip.get("fantasies_detected", "")),
                objections_detected=result.get("objections_detected", latest_skip.get("objections_detected", "")),
                trust_level=result.get("trust_level", int(latest_skip.get("trust_level") or 0)),
                last_confusion=result.get("last_confusion", latest_skip.get("last_confusion", "")),
            )
        return
    ...
    print(f"Stage : {result.get('stage', 'relation')}", flush=True)
    ...
    with get_chat_lock(chat_id):
        ...
        update_user(
            chat_id=chat_id,
            stage=result.get("stage", "relation"),
            client_type=result.get("client_type", "curious"),
            interest_score=result.get("interest_score", 0),
            age_confirmed=result.get("age_confirmed", latest.get("age_confirmed", 0)),
            last_message=clean_text,
            summary=summary,
            username=username,
            first_name=first_name,
            ...
            sexual_profile=result.get("sexual_profile", latest.get("sexual_profile", "")),
            buyer_profile=result.get("buyer_profile", latest.get("buyer_profile", "")),
            fantasies_detected=result.get("fantasies_detected", latest.get("fantasies_detected", "")),
            objections_detected=result.get("objections_detected", latest.get("objections_detected", "")),
            trust_level=result.get("trust_level", int(latest.get("trust_level") or 0)),
            last_confusion=result.get("last_confusion", latest.get("last_confusion", "")),
        )
...
def process_message(message, is_business=False):
    try:
        return _process_message_inner(message, is_business)
    except Exception as exc:
        print("ERREUR PROCESS_MESSAGE:", repr(exc), flush=True)
        traceback.print_exc()
        return
